...
const EditCar = ({ car, setModalEditCar }) => {
  ...
  const [selectedValuesFeature, setSelectedValuesFeature] = useState(
    car?.features ? car.features?.flat() : []
  );
  const [listCate, setListCate] = useState([]);
  const [loading, setLoading] = useState(false);
  const [category, setCategory] = useState(
    car?.model ? JSON.parse(car?.model) : null
  );
  const [arrayFeature, setArrayFeature] = useState([]);
  const { control, handleSubmit } = useForm({
    defaultValues: {
      ...car,
      seat: car?.specifications?.length > 0 ? car.specifications[0][0]?.value : null,
      transmission: car?.specifications?.length > 3 ? car.specifications[3][0]?.value : null,
      fuel: car?.specifications?.length > 2 ? car.specifications[2][0]?.value : null,
      energy: car?.specifications?.length > 1 ? car.specifications[1][0]?.value : null,
    },
  });
  const [image, setImage] = useState(null);
  const [changeImage, setChangeImage] = useState(false);
  const [desc, setDesc] = useState(car?.sortDesc ? car.sortDesc : null);
  ...
  const handleClick = async (value) => {
    ...
    await rentalService.updateCarApp(data, setModalEditCar);
  ...
  return (
    <SafeAreaView className="bg-white h-full">
      <View className=" flex relative mt-[5px] bg-white h-[60px]">
        ...
        <View className="border-[1px] border-[#ddd] py-2 rounded-full absolute w-[40px] h-[40px] left-0 top-[10px] mx-3">
          <TouchableOpacity onPress={() => setModalEditCar(false)}>
        ...
        <View className="">
            ...
            <View className="mb-2 mx-4">
              <Controller
                ...
                render={({
                  ...
                  <>
                    <Animated.View entering={FadeInRight.delay(200).duration(1000).springify()} className="border-[1px] border-[#ddd] p-4 rounded-md w-full ">
                      <TextInput
                        value={value}
          ...
          <View className="">
            ...
            <TouchableOpacity onPress={pickImage}>
              <Animated.View entering={FadeInLeft.delay(200).duration(1000).springify()} className=" mx-4 rounded-lg border-[1px] border-[#ddd] h-[auto] mb-4">
                {!image ? (
                  <View className="border-[1px] border-[#666cff] w-[150px] h-[40px] rounded-md items-center justify-center">
                  ...
                  {allFeatures?.map((item, index) => (
                    <>
                      <TouchableOpacity
                        key={item.id}
                        className="w-[49%] mb-2"
                        onPress={() => toggleValue(item.id, item.name, item.logo)}
                      >
                        <View
                          className={
                            isSelected(item.id)
                  ...
                  <>
                    <View className="border-[1px] border-[#ddd] p-4 rounded-md w-full ">
                      <TextInput
                        ...
                        placeholderTextColor={"gray"}
                        keyboardType="numeric"
                        defaultValue={`${car?.deliveryRadius}`}
                      />
                    </View>
                  ...
                  <>
                    <View className="border-[1px] border-[#ddd] p-4 rounded-md w-full ">
                      <TextInput
                        ...
                        placeholderTextColor={"gray"}
                        keyboardType="numeric"
                        defaultValue={`${car?.deliveryRadiusFree}`}
                      />
                    </View>
                  ...
                  <>
                    <View className="border-[1px] border-[#ddd] p-4 rounded-md w-full ">
                      <TextInput
                        ...
                        placeholderTextColor={"gray"}
                        keyboardType="numeric"
                        defaultValue={`${car?.deliveryPrice}`}
                      />
                    </View>
                  ...
                  <>
                    <View className="border-[1px] border-[#ddd] p-4 rounded-md w-full ">
                      <TextInput
                        ...
                        placeholderTextColor={"gray"}
                        keyboardType="numeric"
                        defaultValue={`${car?.overTimePrice}`}
                      />
                    </View>
                  ...
                  <>
                    <View className="border-[1px] border-[#ddd] p-4 rounded-md w-full ">
                      <TextInput
                        ...
                        placeholderTextColor={"gray"}
                        keyboardType="numeric"
                        defaultValue={`${car?.deodorisePrice}`}
                      />
                    </View>
